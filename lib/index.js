...
function getOptions(options, promiseCallbacks) {
    options = options || {};
    ...
    options.error = function(err, code) {
        ...
        if (error) {
            ...
            var errReason = 'Message: ' + err.message + '\nCode: ' + err.code + '\nFile: ' + err.file + '\nLine: ' + err.line + '\nColumn: ' + err.column;
            pCb.rej(errReason);
        }
    };
 ...
 *
 * Description
 *  - returns a promise eventually fulfilled via what would be the success option callback
 *
 * @param {Object} options
 * @api public
 */

module.exports.pRender = function(options) {
    return new bPromise(function(resolve, reject) {
        if (options.success) {
            throw new Error('Success option not allowed via pRender.  You are given a promise of the result instead.');
        }
        options = getOptions(options, {
            res: resolve,
            rej: reject
